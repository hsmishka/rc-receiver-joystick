...
void loop()
{
  ...
  if (Channel_1_Pulse.available()) {
    chan1val=map(Channel_1_Pulse.width_us(), 1050, 2100, 0, 255);
  }
  if (Channel_2_Pulse.available()) {
    chan2val=map(Channel_2_Pulse.width_us(), 900, 2150, 0, 255);
  }
  if (Channel_3_Pulse.available()) {
    chan3val=map(Channel_3_Pulse.width_us(), 900, 2100, 0, 255);
  }
